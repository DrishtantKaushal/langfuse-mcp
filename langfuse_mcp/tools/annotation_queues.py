...
def register_annotation_queue_tools(mcp, client, enabled_groups: set[str] | None = None):
    ...
    async def list_annotation_queues(
        limit: int = 50, page: int = 1, project: str | None = None,
    ) -> dict:
        """List all annotation queues in the project."""
        return await client.for_project(project).get_annotation_queues(
    ...
    async def create_annotation_queue(
        ...
        return await c.create_annotation_queue(data)

    @mcp.tool()
    async def get_annotation_queue(queue_id: str, project: str | None = None) -> dict:
        """Get a single annotation queue by ID."""
        return await client.for_project(project).get_annotation_queue(queue_id)
    ...
    async def list_annotation_queue_items(
        ...
        return await c.get_annotation_queue_items(queue_id, **params)

    @mcp.tool()
    async def get_annotation_queue_item(
        queue_id: str, item_id: str, project: str | None = None,
    ) -> dict:
        """Get a single queue item by ID."""
        return await client.for_project(project).get_annotation_queue_item(
            queue_id, item_id
        )
    ...
    async def create_annotation_queue_item(
        ...
        if status:
            data["status"] = status
        return await c.create_annotation_queue_item(queue_id, data)
    ...
    async def update_annotation_queue_item(
        ...
        return await client.for_project(project).update_annotation_queue_item(
    ...
    async def delete_annotation_queue_item(
        ...
        return await client.for_project(project).delete_annotation_queue_item(
            queue_id, item_id
        )

    @mcp.tool()
    async def create_annotation_queue_assignment(
        queue_id: str, user_id: str, project: str | None = None,
    ) -> dict:
        """Assign a reviewer (user) to an annotation queue."""
        return await client.for_project(project).create_annotation_queue_assignment(
            queue_id, user_id
        )
    ...
    async def delete_annotation_queue_assignment(
        ...
        return await client.for_project(project).delete_annotation_queue_assignment(
            queue_id, user_id
        )
